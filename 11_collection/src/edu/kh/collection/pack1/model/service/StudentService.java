...
public class StudentService {
	...
	*/
	...
	/**
	 * 6. 이름에 문자열이 포함되는 학생을 찾아서 조회하는 메서드
	 * 
	...
	public void searchName2() {
		...
		for(Student std : studentList) {
			// 순서 유의
			// boolean String.contains(문자열) : String에 문자열 이 포함되어있으면 true/false
			if(std.getName().contains(input)) { // std.getName()에 input이 포함되어있는가?
				System.out.println(std);
	 ...
	 * Student에 Comparable 인터페이스를 상속받아 오버라이딩한  compareT()에
	 * 정의한 대로 정렬됨(나이 오름차순, 내림차순..)
	 * 
	 * 방법 2: Comparator 클래스에 의한 정렬 compare()사용 (익명 내부 클래스 이용)
	 * 익명 내부 클래스 : 이름이 없는 클래스를 즉석에서 선언해서 한번만 사용할 목적으로 이용
	 * 객체를 생성하면서 바로 구현 내용을 정의할 수 있음
	 * 익명 내부클래스 장점
	 * 코드 간결화(별도로 클래스를 만들지 않아도 될 때 사용)
	 * 즉시 사용(한번만 사용할 Comparator 등을 정의할 때 유용함)
	...
	public void sortByName() {
									// 익명 내부클래스는 Comparator 인터페이스를 상속
									// 구현한 구현체(== 클래스)
		Collections.sort(studentList, new Comparator<Student>(){
			...
			public int compare(Student o1, Student o2) {
				...
				// (String이 Coparable을 상속받아 재정의해둔 compareTo() 메서드를 이용하는 것)

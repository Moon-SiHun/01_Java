package edu.kh.excption.model.service;

import java.io.FileNotFoundException;
import java.io.IOException;
import java.util.InputMismatchException;
// ArithmeticException : java.lang에 속해있어서 import 하지 않아도 됨
...
public class ExceptionService {
	...
	public void ex2() {
		...
		//				ㄴInputMismatchException, ArithmeticException
			...
			// 다형성 - 업캐스팅 : 부모타입 참조변수 자식객체 참조
			
			// ** 상위 타입의 예외 클래스를 catch문에 작성하면
			// 다형성 업캐스팅에 의해 모두 잡아서 처리 가능 **
			System.out.println("무엇인지 모르겠으나 예외가 발생해서 처리함");
			e.printStackTrace();
	...
	public void ex3() {
		// try - catch - finally
		// finally : try 구문에서 예외가 발생하든 안하든 무조건 마지막에 수행
			try {System.out.print("정수 1 입력 :"); 
			int input1 = sc.nextInt();
			// int형이 아닌 다른 자료형을 대입했을 때

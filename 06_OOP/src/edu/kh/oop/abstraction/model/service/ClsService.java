...
//기본 생성자 작성하지 않은 경우, 
//클래스 사용시 컴파일러가 자동으로 기본 생성자 생성
public class ClsService /*extends Student*/{
			// 부모로 Student 클래스를 상속받음
			// ClsSerice(자식) <- Student(부모)
	...
	public void ex1() {
			...
			// -> 전체에서 직접 접근 가능한 public인 v1만 에러발생하지 않음
			//System.out.println(std.v2);
			//System.out.println(std.v3);
			//System.out.println(std.v4);
			
			// 상속관계에서 직접 접근 가능 범위 테스트
			// * 상속받으면 부모의 것도 내 것처럼 사용
			//System.out.println(v1);
			//System.out.println(v2);
			// Student 클래스를 ClsService가 상속 받았으므로
			// Student의 protected 변수인 v2는 직접접근 가능
			
			//System.out.println(v3); // (default) 다른 패키지라 불가능
			//System.out.println(v4); // 해당 클래스 내부가 아닌 다른 클래스라서 불가능
	...
	public void ex2() {
		...
		// schoolName에 노란줄이 뜨는 이유9(경고) => 제대로 작성을 안해서
	...
	public void ex4() {
		...
		User u3 = new User("user03", "pass03", "김길동", 30, '남'); //매개변수 5개짜리 생성자(전부 초기화)
